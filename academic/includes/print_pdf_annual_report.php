...
EOD;
        ...
        while ($SEMcounter < 3) {
            ...
            $qtotalstdsem = mysqli_query($zalongwa, "SELECT COUNT(DISTINCT(CourseCode)) AS TOTAL FROM examresult 
											WHERE RegNo='$regno' and AYear='$year' AND Semester='$SEMcounter'");
            list($totalstdsem) = mysqli_fetch_array($qtotalstdsem);
            ...
            $courseInvolved = 0;
            while (list($studycourse, $option) = mysqli_fetch_array($dbstdcourse)) {
...
EOD;
    ...
    $i = 1;
    $qcourse = mysqli_query($zalongwa, "SELECT DISTINCT CourseCode FROM courseprogramme WHERE  (ProgrammeID='$deg') 
								AND (YearofStudy='$yearofstudy') AND (AYear='$year') ORDER BY CourseCode");

    while (list($course) = mysqli_fetch_array($qcourse)) {
