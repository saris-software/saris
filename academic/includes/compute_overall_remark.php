...
$dbcoursecount = mysql_query($qcoursecount) or die (mysql_error().'mimi na wewe');
$row_coursecount = mysql_fetch_array($dbcoursecount);
...
if($semval == 1){
	...
	$avg = substr($gmarks/$subjecttaken,0,3);
	$gpa = substr($sem1totalsgp/$sem1unittaken,0,3);
}elseif($semval == 2){
	$sem2totalcredits = $coursecount;
	$ptc=$sem2unittaken/$sem2totalcredits*100;
	$avg = substr($gmarks/$subjecttaken,0,3);
	$gpa = substr($sem2totalsgp/$sem2unittaken,0,3);
}
	...
	if(($unittaken>0)&&($totalinccount==0))
	{
		...
		$x=$x+235;
			if($reportgroup == 'sheet'){
				$pdf->line($x+$cw, $y, $x+$cw, $y+15);	$pdf->text($x+$cw+4, $y+12, $unittaken);  #print total subject candidate taken
				$pdf->line($x+$cw+30, $y, $x+$cw+30, $y+15);	$pdf->text($x+$cw+32, $y+12, $totalsgp); 
				$pdf->line($x+$cw+60, $y, $x+$cw+60, $y+15);	$pdf->text($x+$cw+63, $y+12, $gpa); 	
			}
			...
			if($reportgroup == 'annualsheet'){
				if($semval == 2)
				{
					...
					if($ovavg>0){
						$pdf->text($x+$cw+335-$ovrw, $y+12, number_format($ovgpa,1,'.','')); 		
					}			
				}elseif($semval == 1)
				{
		...
		$x=$x+235;
			if($reportgroup == 'sheet'){
				$pdf->line($x+$cw, $y, $x+$cw, $y+15);	$pdf->text($x+$cw+4, $y+12, $unittaken);  
				$pdf->line($x+$cw+30, $y, $x+$cw+30, $y+15);	$pdf->text($x+$cw+32, $y+12, $totalsgp); 
				$pdf->line($x+$cw+60, $y, $x+$cw+60, $y+15);	$pdf->text($x+$cw+63, $y+12, $gpa); 
		    ...
		    if($reportgroup == 'annualsheet'){
		   		if($semval == 2)
				{
					$pdf->line($x+$cw, $y, $x+$cw, $y+15);	$pdf->text($x+$cw+4, $y+12, $gpa);  
					$sem2avg= $avg;
					$sem2gpa= $gpa;
					$pdf->line($x+$cw+30, $y, $x+$cw+30, $y+15);	
					$pdf->line($xpoint+25, $y, $xpoint+25, $y+15); 
					...
					$ovgpa = ($sem1gpa+$sem2gpa)/2;
					if($ovavg>0){
						$pdf->text($x+$cw+336-$ovrw, $y+12, number_format($ovgpa,1,'.','')); 	
					}
				}elseif($semval == 1)
				{
