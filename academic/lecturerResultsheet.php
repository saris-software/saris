...
if (isset($_POST['PDF']) && ($_POST['PDF'] == "Print PDF")){
	...
	if (($checkdegree=='on') && ($checkyear == 'on') && ($checkcohot == 'on')){
		...
		$qstd = "SELECT DISTINCT courseprogramme.coursecode FROM courseprogramme INNER JOIN examresult ON (courseprogramme.CourseCode = examresult.CourseCode) WHERE  (ProgrammeID='$deg') AND (AYear='$year') AND (Semester='$sem') ORDER BY CourseCode";
		$dbstd = mysqli_query($zalongwa, $qstd);
		$totalcolms = mysqli_num_rows($dbstd);
		...
		while($rowstd = mysql_fetch_array($dbstd)) {
			...
			$qstdcourse = "SELECT DISTINCT coursecode FROM courseprogramme INNER JOIN examresult ON (courseprogramme.CourseCode = examresult.CourseCode) WHERE  (ProgrameID='$deg') AND (AYear='$year') AND (Semester='$sem') ORDER BY CourseCode";
		...
		*/
		...
		$i=1;
			...
			while($rowstudent = mysqli_fetch_array($dbstudent)) {
				...
				$qcourse = "SELECT DISTINCT courseprogramme.CourseCode FROM courseprogramme INNER JOIN examresult ON 
							(courseprogramme.CourseCode = examresult.CourseCode) WHERE  (ProgrammeID='$deg') AND (examresult.AYear='$year') AND 
				...
				$total_rows = mysqli_num_rows($dbcourse);
					...
					$header = $x+235;
							...
							$qcoursestd="SELECT DISTINCT course.Units, course.Department, course.StudyLevel, examresult.CourseCode FROM 
							...
							$name = $row_coursestd['CourseName'];
							$coursefaculty = $row_coursestd['Department'];
					...
					while($row_course = mysqli_fetch_array($dbcourseUnit)){
						...
						$cw = $cwf*$totalcolms;
						$x=$x+235;
								$pdf->line($x+$cw, $y, $x+$cw, $y+15);	$pdf->text($x+$cw+4, $y+12, $total_rows);  #print total subject candidate taken
								$pdf->line($x+$cw+30, $y, $x+$cw+30, $y+15);	$pdf->text($x+$cw+32, $y+12, $gmarks); 
								$pdf->line($x+$cw+60, $y, $x+$cw+60, $y+15);	$pdf->text($x+$cw+67, $y+12, $avg); 
								#get student remarks
...
if($space<70){
 ...
 #print statistics
 //$pdf->text(50, $yind, 'PASS count = '.$overallpasscount.' SUPP Count = '.$overallsuppcount.' ABSC Count ='.$overallinccount );
 	/*
