...
if (isset($_POST['PDF']) && ($_POST['PDF'] == "Print PDF")){
	...
	if (($checkdegree=='on') && ($checkyear == 'on') && ($checkcohot == 'on')){
		...
		$pdf->text($x+250, $y+14, 'ACADEMIC YEAR: '.$year.' - '.$sem); 
		$pdf->text($x+200, $y+28, 'ANNUAL OVERALL EXAMINATIONS RESULTS');
		...
		$pdf->line($x+$cw+60, $y, $x+$cw+60, $y+15);	$pdf->text($x+$cw+62, $y+12, 'GPA I&II'); 
		...
		$i=1;
			while($rowstudent = mysqli_fetch_array($dbstudent)) {
					...
					$qcourse="SELECT DISTINCT course.Units, course.Department, course.StudyLevel, examresult.CourseCode FROM 
					 ...
					 ORDER BY examresult.CourseCode";	
					...
					$pdf->setFont('Arial', '', 7); 
					$pdf->line($x, $y, $xpoint+25, $y); 
					$pdf->line($x, $y+45, $xpoint+25, $y+45); 
					...
					$clmspace = $x+200; 
						...
						$x=$x+200;
								...
								$pdf->line($x+$cw+95, $y, $x+$cw+95, $y+45);	if ($igrade<>'I'){$pdf->text($x+$cw+97, $y+14, $remark);
								}
								$pdf->line($xpoint+25, $y, $xpoint+25, $y+45); 
...
if($space<70){
	...
	$pdf->text(60, $yind+24, 'Signature of The Secretary, Examination Board:  �����������������     ');  
	$pdf->text(330, $yind+24, 'Date: �������      ');
 	$pdf->text(489, $yind+24, 'Signature of the Chairperson, Examination Board:  ������');  
	$pdf->text(739, $yind+24, 'Date: �����');  

	$pdf->text(290.28, $yind, '          ######## END OF EXAM RESULTS ########');   
...
#get connected to the database and verfy current session
	...
	$szSubSection = 'NTA Annual Rpt';
	$szTitle = 'NTA System Annual Report Examination Results';
