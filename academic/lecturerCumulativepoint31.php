...
$i=1;
	while($rowstudent = mysql_fetch_array($dbstudent)) {
			...
			$mysq = mysql_query("SELECT DISTINCT AYear FROM examresult WHERE RegNo='$regno' ORDER BY AYear ASC");
			$stud_rows = mysql_num_rows($mysq);
			$stud_val = mysql_fetch_array($mysq);
			if($stud_rows < 1){
				#do nothing
				}
			else{
				...
				for($x=1; $x<=$stud_rows; $x++){
					...
					echo"</tr><tr>";
				$key = $regno;
				while($yrs = mysql_fetch_array($mysq)){					
					$acyr = $yrs['AYear'];
					# get all courses for this candidate
