...
While ($row_dusstudent = mysql_fetch_assoc($dbdusstudent))
{
	...
	#find programmename
	$qdegree = "select * from programme where ProgrammeCode ='$prog'";
	...
	*/
	...
	$dbresult = mysql_query($qresult);
	$dbcount = mysql_query($qresult) ;
	$count = mysql_num_rows($dbcount);
	if ($count > 0)
	{
		#insert student record
		...
		while ($row_result = mysql_fetch_assoc($dbresult))
		{
			...
			$qinsertexam = "INSERT INTO allexamresults SET 
							...
							AYear = '$year'
							";
			$dbinsertexam =mysql_query($qinsertexam) or die (mysql_error());
		}
		*/
	}
}
