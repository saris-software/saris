...
if (isset($_POST["save"])) {
    ...
    $get_postName = mysqli_query($zalongwa, "SELECT Post FROM electionpost WHERE Id='$postid'");
...
if (isset($_POST["add"])) {
    ...
    if ($today > $enddate && mysqli_num_rows($dbedate) > '0') {
        ...
        $qpostname = mysqli_query($zalongwa, "SELECT Post FROM electionpost WHERE Id='$post'");
    ...
    <fieldset>
        ...
        <form action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post" enctype="multipart/form-data" name="frmAyear"
              target="_self">
