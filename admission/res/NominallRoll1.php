...
Nominal Roll <?php echo $ayear;?> Academic Year
</td>
...
$query=mysql_query("select * from student where ProgrammeofStudy='$p[ProgrammeCode]' and EntryYear='$e[EntryYear]' limit 200");
echo"<tr class='dhead'>";
echo"<th>S/No</th>
<th>Name</th>
<th>SEX</th>
...
$unknown=mysql_query("SELECT *  FROM student WHERE Faculty='$sm[FacultyName]' and (Sex !='F' and Sex !='M')");
$syn=mysql_num_rows($unknown);
//Count Genders
...
///Mkopo
$hslb=mysql_query("SELECT count(Name)as hslb FROM student WHERE Faculty='$sm[FacultyName]'and Sponsor='$sponsors'");
$others=mysql_query("SELECT count(Name)as other  FROM student WHERE Faculty='$sm[FacultyName]' and (Sponsor !='$sponsors')");
...
$Total_main=$T_men+$T_women+$T_unknown;
$T_all=$T_hslb+$T_other;
echo "<tr>
<td colspan='2'>SUB TOTAL</td><td>$T_men</td><td>$T_women</td><td>$T_unknown</td>
<td>&nbsp;</td>
<td>$T_hslb</td>
<td>$T_other</td>
<td>&nbsp;</td>
...
<th >HISTOGRAM SHOWING NUMBER OF STUDENTS AGAINST DEPARTMENTS.</th>
</tr></table>
<br>
