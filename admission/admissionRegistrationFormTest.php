...
#get post variables
    ...
    $totalRows_update = mysqli_num_rows($update) or die(mysqli_error($zalongwa));
...
//FORMATING ERRORS
    if (!$formsix || !$formfour || !$diploma || !$phone1 || !$email1 || !$regno || !$degree || !$faculty || !$ayear || !$combi || !$campus || !$manner || !$byear || !$bmon || !$bday || !$dtDOB || !$surname || !$firstname || !$dtDOB || !$age || !$sex || !$sponsor || !$country || !$maritalstatus || !$address || !$religion || !$denomination || !$postaladdress || !$residenceaddress || !$status || !$gyear || !$name) {
        if (!$regno) {
            $regno_error = "<span style=\"color: red; \">*Registration Number Must be Filled</span>";
        }
        if (!$phone1) {
...
//$formsix_error="<font color='red'>*Form Six Necta Number Must be Filled</font>";
        ...
        if (!$formfour) {
            $formfour_error = "<span style=\"color: red; \">*Form Four Necta Number Must be Filled</span>";
        }
...
//$diploma_error="<font color='red'>*Diploma Necta Number Must be Filled</font>";
        ...
        if (!$degree) {
            $degree = "<span style=\"color: red; \">*Degree Must be Filled</span>";
        }


        if (!$faculty) {
            $faculty_error = "<span style=\"color: red; \">*Faculty  Must be Filled</span>";
        }

        if (!$ayear) {
            $ayear_error = "<span style=\"color: red; \">*Date Must be Filled</span>";
        }

        if (!$combi) {
            $combination_error = "<span style=\"color: red; \">*Combination  Must be Filled</span>";
        }
        if (!$campus) {
            $campus_error = "<span style=\"color: red; \">*Campus Name Must be Filled</span>";
        }
        if (!$manner) {
            $manner_error = "<span style=\"color: red; \">*Manner Must be Filled</span>";
        }


        if (!$byear) {
            $byear_error = "<span style=\"color: red; \">*Birth Date Must be Filled</span>";
        }
        if (!$bmon) {
            $bmon_error = "<span style=\"color: red; \">*Month Must be Filled</span>";
        }
        if (!$sex) {
            $sex_error = "<span style=\"color: red; \">*Gender Must be Filled</span>";
        }

        if (!$bday) {
            $bday_error = "<span style=\"color: red; \">*Birth day Must be Filled</span>";
        }
        if (!$dtDOB) {
        ...
        */
        ...
        if (!$age) {
            $age_error = "<span style=\"color: red; \">*Age Must be Filled</span>";
        }
        ...
        */
        if (!$region) {
            $region_error = "<span style=\"color: red; \">*Region Must be Filled</span>";
        }
        if (!$maritalstatus) {
        ...
        if (!$religion) {
            $religion_error = "<span style=\"color: red; \">*Religion Must be Filled</span>";
        }
        if (!$denomination) {
            $denomination_error = "<span style=\"color: red; \">*Denomination Must be Filled</span>";
        }
        if (!$postaladdress) {
        ...
        */
        ...
        if (!$gyear) {
            $gyear_error = "<span style=\"color: red; \">*Gyear Address Must be Filled</span>";
        }
        if (!$name) {
            $name_error = "<span style=\"color: red; \">*Name Address Must be Filled</span>";
        }
    ...
    <form action=" <?php echo $_SERVER['PHP_SELF'] ?>" method="POST" name='admission'>
        ...
        <table cellpadding='0' cellspacing='0' bgcolor='#DBDBDB' class='ztable'>
            ...
            <tr>
                ...
                <td class='ztable'>
                    <input name="AdmissionNo" type="text" id="AdmissionNo"
                           value="<?php echo $AdmissionNo; ?>" <?php echo $state4; ?> <?php echo $state3; ?>>
                    <?php ?>
                </td>
            </tr>
            <tr>
            ...
            <tr>
                <td class='formfield'>Faculty:<span class="style2">*</span>
                </td>
                <td class='ztable'>
            ...
            <tr>
                ...
                <td class='ztable'>
                    <input name="middlename" type="text" id="middlename" value="<?php echo $middlename; ?>" size="15"
                           maxlength="50" <?php echo $state3; ?> <?php echo $state4; ?>/>
            ...
            <tr>
                ...
                <td class='ztable'>
                    <input name="firstname" type="text" value="<?php echo $firstname; ?>" id="firstname"
            ...
            <tr>
                <td class='formfield' norwap>Sex:<span class="style2">*</span></td>
                <td class='ztable'>
                ...
                <td nowrap="nowrap" class="ztable">
                    <input name="paddress" type="text" id="paddress" size="30"
            ...
            <tr>
                ...
                <td class='ztable'>
                    <input name="district" type="text" id="district" value="<?php echo $district; ?>"
                           size="30" <?php echo $state4; ?> />
            ...
            <tr>
                ...
                <td class='ztable'>
                    <input name="region" type="text" id="region" size="30"
            ...
            <tr>
                ...
                <td class='ztable'>
                    <select name="select" <?php echo $state4; ?>>
                        ...
                        $countrys = mysqli_query($query_country) or die(mysqli_error());
            ...
            <tr>
                ...
                <td class='ztable'>
                    <input name="bank_branch_name" size="30"
            ...
            <tr>
                ...
                <td class='ztable'>
                    <input name="account_number" size="30"
            ...
            <tr>
                ...
                <td nowrap="nowrap" class='ztable'>
                    <input name="form6no" type="text" id="formsix"
            ...
            <tr>
                ...
                <td nowrap="nowrap" class='ztable'>
                    <input name="form7no" type="text" id="diploma"
...
where RegNo='$regno'";
        $dbstudent = mysqli_query($zalongwa, $sql) or die(mysqli_error());
        if (!$dbstudent) {
...
function uploader()
{
    ...
    $Filelink = $FileID . ".gif";
    $folder = "images";
    $test = mysqli_query("SELECT * FROM Student WHERE regno='$regno'");
    $n = mysqli_num_rows($test);
    if ($n == 1) {
        foreach ($_FILES as $file_name => $file_array) {
            ...
            if (($Type == "image/bmp") || ($Type == "image/x-png") || ($Type == "image/gif") || ($Type == "image/pjpeg")) {
                if (is_uploaded_file($file_array['tmp_name'])) {
                    $upd = mysqli_query("update student set Photo='$FileID'  where  regno='$regno'") or die(mysql_error());
                    move_uploaded_file($file_array['tmp_name'], "$folder/$Filelink") or die ("Couldn't copy");
                    echo "File Successfuly Uploaded<br><br>";
                }
            } else {
                upload($regno);
                echo "<span style=\"color: red; \">File Type is not supported</span>";
            }
        }
    } else
