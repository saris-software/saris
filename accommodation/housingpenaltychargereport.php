...
#get connected to the database and verfy current session
	...
	$szSubSection = 'Penalty Charges';
	$szTitle = 'Penalty Charges';
	...
	$std = mysqli_query($zalongwa,$query_paytype) or die(mysqli_error());
  ...
  // Add send functionality here
		...
		if(!empty($receiptno)){
			...
			if ($num_row > 0) {
			  ...
			  <fieldset>
				<legend>Penalty Charges Registration Form</legend>
...
$qrefunded = "SELECT * FROM tblcautionfee WHERE (RegNo = '$key' AND Paytype='3') Order By Received Desc";
$refunded = mysqli_query($zalongwa,$qrefunded);
$num_row_refunded = mysqli_num_rows($refunded);
if ($num_row_refunded > 0) {
		?>
		This Candidate was Previously Penalised the Following:
