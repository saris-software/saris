...
if (isset($_POST['printPDF']) && ($_POST['printPDF'] == "Print PDF")) {
	...
	if ($all =='on'){
		...
		student.Faculty, 
		roomapplication.RegNo, 
		roomapplication.Hall, 
		roomapplication.AppYear, 
		criteria.ShortName
		FROM (student INNER JOIN roomapplication ON student.RegNo = roomapplication.RegNo) 
		INNER JOIN criteria ON roomapplication.AllCriteria = criteria.CriteriaID
		WHERE (((roomapplication.AppYear)='$year')) ORDER BY student.ProgrammeofStudy, student.RegNo ";
	}else{
		$sql = "SELECT student.Name, 
		student.ProgrammeofStudy, 
		student.Faculty, 
		...
		FROM (student INNER JOIN roomapplication ON student.RegNo = roomapplication.RegNo) 
		INNER JOIN criteria ON roomapplication.AllCriteria = criteria.CriteriaID
		WHERE (roomapplication.AppYear='$year') AND (criteria.ShortName = '$citeria') ORDER BY student.ProgrammeofStudy, student.RegNo";
	 }
		...
		#print header
		$pdf->image('images/logo.jpg', 288, 50);   // Image at x=50 and y=200. 
		$pdf->setFont('Arial', 'I', 8);     // Set font to arial bold italic 12 pt. 
		$pdf->text(530.28, 825.89, 'Page '.$pg);   // Text at x=100 and y=100. 
		$pdf->setFont('Arial', 'BI', 30);     // Set font to arial bold italic 12 pt. 
		$pdf->setFillColor('rgb', 0, 0, 1);   // Set text color to blue. 
		$pdf->text(66, 50, 'UNIVERSITY OF DAR ES SALAAM');   // Text at x=100 and y=100. 
		$pdf->setFillColor('rgb', 0, 0, 0);   // Set text color to blue. 
		
		#University Addresses
		$pdf->setFont('Arial', '', 11.3);     // Set font to arial bold italic 12 pt. 
		$pdf->text(115, 70, 'Phone:  2410500/8');   // Text at x=100 and y=100. 
		$pdf->text(115, 82, 'Fax: 255-022-2410029');   // Text at x=100 and y=100. 
		$pdf->text(115, 94, 'Email: usab@udsm.ac.tz');   // Text at x=100 and y=100. 
		$pdf->text(350, 70, 'P.O. Box 35091');   // Text at x=100 and y=100. 
		$pdf->text(350, 82, 'DAR ES SALAAM');   // Text at x=100 and y=100. 
		$pdf->text(350, 94, 'http://www.usab.udsm.ac.tz');   // Text at x=100 and y=100. 
		
		$pdf->setFillColor('rgb', 0, 0, 0);   // Set text color to blue. 
		$pdf->setFont('Arial', '', 13);     // Set font to arial bold italic 12 pt. 
		$pdf->text(150, 132, $year.' - ROOM APPLICATION REPORT'); // Text at x=100 and y=200.
		$pdf->setFillColor('rgb', 0, 0, 0);   // Set text color to blue. 
		
		$pdf->setFillColor('rgb', 0, 0, 0);   // Set text color to blue. 
		$pdf->setFont('Arial', '', 13);     // Set font to arial bold italic 12 pt. 
		$pdf->text(70, 150, $row_result["ShortName"]); // Text at x=100 and y=200.
		$pdf->setFillColor('rgb', 0, 0, 0);   // Set text color to blue. 
...
@$citeria =$_POST['criteria'];
	...
	if ($all =='on'){
		$sql = "SELECT student.Name,  roomapplication.RegNo, roomapplication.Hall, roomapplication.AppYear, criteria.ShortName
		FROM (student INNER JOIN roomapplication ON student.RegNo = roomapplication.RegNo) INNER JOIN criteria ON roomapplication.AllCriteria = criteria.CriteriaID
		WHERE (((roomapplication.AppYear)='$year')) ORDER BY student.Name";
	}else{
		$sql = "SELECT student.Name, student.Sex, roomapplication.RegNo, roomapplication.Hall, roomapplication.AppYear, criteria.ShortName
		FROM (student INNER JOIN roomapplication ON student.RegNo = roomapplication.RegNo) INNER JOIN criteria ON roomapplication.AllCriteria = criteria.CriteriaID
		WHERE (roomapplication.AppYear='$year') AND (criteria.ShortName = '$citeria') ORDER BY student.Name";
	 }
	$result = @mysqli_query($zalongwa,$sql) or die("Cannot query the database.<br>" . mysqli_error());
	$query = @mysqli_query($zalongwa,$sql) or die("Cannot query the database.<br>" . mysqli_error());
	...
	if (mysqli_num_rows($query) > 0)
	{
		echo '<b>Allocation Criteria: </b>'.$citeria;
		echo "<p><b>Total Applications:</b> $totalRows_query </p>";
		echo "<table border='1'cellpadding='0' cellspacing='0' bordercolor='#006600'>";
		echo "<tr><td> S/No </td><td> Name </td><td> RegNo </td><td> Sex </td><td> Criteria </td><td>Description</td></tr>";
		$i=1;
		while($result = mysqli_fetch_array($query))
		{
				$Name = stripslashes($result["Name"]);
...
<form action="<? echo $editFormAction; ?>" method="POST" name="studentRoomApplication" id="studentRoomApplication">
