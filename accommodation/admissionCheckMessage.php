                ...
                <td><?php $from=$row_studentsuggestion['fromid']; 
						  ...
						  $id=$row_studentsuggestion['id']; 
								...
								if($rows = mysqli_num_rows($dbstudent) != 0){
									...
									$user = mysqli_query("SELECT * FROM security WHERE RegNo='$from'");
...
<p><a href="<?php printf("%s?pageNum_studentsuggestion=%d%s", $currentPage, max(0, $pageNum_studentsuggestion - 1), $queryString_studentsuggestion); ?>">Previous</a>
			...
			of <?php echo $totalRows_studentsuggestion ?> <span class="style64 style1">...</span><a href="<?php printf("%s?pageNum_studentsuggestion=%d%s", $currentPage, min($totalPages_studentsuggestion, $pageNum_studentsuggestion + 1), $queryString_studentsuggestion); ?>">Next</a> 
			<span class="style64 style1">.......</span><?php echo "<a href=\"admissionSuggestionBox.php?from=$from&id=$id\">Reply Message</a>" ?></p>
