...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "frmInst")) {
  $insertSQL = sprintf("INSERT INTO programme (ProgrammeName, Title, Faculty) VALUES (%s, %s, %s)",
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "frmInstEdit")) {
  $updateSQL = sprintf("UPDATE programme SET ProgrammeName=%s, Title=%s, Faculty=%s, CampusID=%s WHERE ProgrammeID=%s",
                       GetSQLValueString($_POST['txtName'], "text"),
                       GetSQLValueString($_POST['txtHead'], "text"),
                       GetSQLValueString($_POST['cmbFac'], "text"),
...
mysqli_select_db($zalongwa,$database_zalongwa);
$query_inst = "SELECT * FROM programme";
...
<table border="1" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <td><strong>Degree Code </strong></td>
	<td><strong>Full Name</strong></td>
  </tr>
  <?php do { ?>
  <tr>
    <td nowrap><?php $id = $row_inst['ProgrammeID']; $name = $row_inst['Faculty'];
	echo "<a href=\"admissionProgramme.php?edit=$id\">$name</a>"?></td>
    <td nowrap><?php echo $row_inst['ProgrammeName']; ?></td>
	 <td><?php echo $row_inst['Title']; ?></td>
  </tr>
...
?>
    ...
    <tr bgcolor="#CCCCCC">
      <th nowrap scope="row"><div align="right">Degree Code:</div></th>
      <td><input name="txtName" type="text" id="txtName" value="<?php echo $row_instEdit['ProgrammeName']; ?>" size="40"></td>
    </tr>
    <tr bgcolor="#CCCCCC">
      <th nowrap scope="row"><div align="right">Full Name:</div></th>
      <td><input name="txtHead" type="text" id="txtHead" value="<?php echo $row_instEdit['Title']; ?>" size="40"></td>
    </tr>
    <tr bgcolor="#CCCCCC">
